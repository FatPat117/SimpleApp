...
Backend API cho hệ thống xác thực người dùng, tập trung vào bảo mật session bằng `HttpOnly Cookie`, luồng xác thực email, quên mật khẩu với mật khẩu tạm, và tích hợp đăng nhập Google OAuth.
...
- Tích hợp gửi email xác thực và email mật khẩu tạm qua SMTP, có hàng đợi RabbitMQ để xử lý bất đồng bộ.
...
- `POST /auth/signup`: đăng ký tài khoản, gửi email xác thực.
- `GET /auth/signup/availability`: kiểm tra email/username đã tồn tại chưa.
- `GET /auth/verify-email?token=...`: xác thực email từ link gửi qua mail.
...
- `POST /auth/forgot-password`: gửi mật khẩu tạm qua email.
...
- Email chưa xác thực thì không cho login.
...
### Bước 2: Chuẩn bị dịch vụ ngoài (Postgres + RabbitMQ)
...
- Chạy migration
- Chạy seeder
- Khởi động consumer RabbitMQ cho mail queue
...
- Đang dùng queue mail với cơ chế fallback: nếu enqueue lỗi sẽ gửi mail trực tiếp.
